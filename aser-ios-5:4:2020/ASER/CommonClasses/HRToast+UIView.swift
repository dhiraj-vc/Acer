...
extension UIView {
    ...
    */
    ...
    class func hr_toastFontColor() -> UIColor {
        ...
        if color == nil {
            color = Colours.AppColor //UIColor.white
    ...
    func makeToast(message msg: String) {
        self.makeToast(message: msg, duration: HRToastDefaultDuration, position: HRToastPositionTop as AnyObject)
    }
    ...
    fileprivate func showToast(_ toast: UIView, duration: Double, position: AnyObject) {
        ...
        if HRToastHidesOnTap {
            _ = UITapGestureRecognizer(target: toast, action: #selector(UIView.handleToastTapped(_:)))
    ...
    fileprivate func centerPointForPosition(_ position: AnyObject, toast: UIView) -> CGPoint {
        if position is String {
            ...
            if position.lowercased == HRToastPositionTop {
                return CGPoint(x: viewSize.width/2, y: toastSize.height/2 + HRToastVerticalMargin + 20)
