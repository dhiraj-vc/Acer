...
//    var surveyVCObj = SurveyQuestionVC()
    ...
    {
        if KAppDelegate.loginTypeVal == 1
        {
            WebServiceProxy.shared.postData("\(Apis.KAddSurveyQuestion)", params: param, showIndicator: true)  { (ApiResponse) in
    ...
    func handelAddResponse(_ response:ApiResponse,view:UIViewController){
        if response.success {
            if KAppDelegate.loginTypeVal == UserRole.User.rawValue {
                ...
                Proxy.shared.pushToNextVC(identifier:"CreateResearchProgramVC", isAnimate: false, currentViewController: view)
            }
            KAppDelegate.arrForQuestion.removeAllObjects()
...
extension SurveyQuestionVC :UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate, UIPickerViewDelegate, UIPickerViewDataSource{
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if tableView==listtblvw{
            return arrQuestionsFiltered.count
        }else{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if tableView==listtblvw{
            let cell = listtblvw.dequeueReusableCell(withIdentifier: "ListViewCell") as! ListViewCell
            cell.questionLbl.text = arrQuestionsFiltered[indexPath.row].question
            return cell
        }else{
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        if tableView == listtblvw{
            ...
            headerView.txtFldOptions.tag = section
            headerView.txtFldPictureSubmission.tag = section
            if let dict = KAppDelegate.arrForQuestion[section] as? NSDictionary {
            ...
            headerView.frame = CGRect(x: 0, y: 0, width: tableView.bounds.width, height: 142)
...
//                let optionDict = arrOptions[IndexPath] as! NSDictionary

//                let optionAns = optionDict[ "an"] as! String
...
//                print(optionAns)
                ...
                if questionType == "\(Options.TYPE_ANS.rawValue)" || questionType == "\(Options.NO_OPTION.rawValue)"{
                    footerView.viewForButton.isHidden = true
                    footerView.cnstHeightForView.constant = 0
                    footerView.frame = CGRect(x: 0, y: 0, width: tableView.bounds.width, height: 60)
                } else {
                    footerView.viewForButton.isHidden = false
                    footerView.cnstHeightForView.constant = 30
                    footerView.frame = CGRect(x: 0, y: 0, width: tableView.bounds.width, height: 90)
                }
            }
            footerView.btnRemoveQues.tag = section
            footerView.btnAddQues.tag = section
            footerView.mandatoryBtn.tag = section
            
            if KAppDelegate.arrForQuestion.count == 1{
                footerView.btnRemoveQues.isHidden = true
            } else {
                footerView.btnRemoveQues.isHidden = false
            }
            
            if mandForQuestion.count>0{
                if mandForQuestion .contains(section){
                    footerView.mandatoryBtn.setImage(UIImage(named:"ic_radio_selected"), for: .normal)
                }else{
                    footerView.mandatoryBtn.setImage(UIImage(named:"ic_radio_unselected"), for: .normal)
                }
            }
            
            footerView.btnRemoveQues.addTarget(self, action: #selector(removeQuestionnaire(sender:)), for: .touchUpInside)
            footerView.btnAddQues.addTarget(self, action: #selector(addOptions(sender:)), for: .touchUpInside)
             footerView.mandatoryBtn.addTarget(self, action: #selector(addMandatory(sender:)), for: .touchUpInside)
...
//            cell.btnCorrectAnswer.addTarget(self, action: #selector(ansForOptions(sender:)), for: .touchUpInside)
    ...
    func tableView(_ tableView: UITableView, heightForFooterInSection section: Int) -> CGFloat {
        if tableView == listtblvw{
            ...
            if let dict = KAppDelegate.arrForQuestion[section] as? NSDictionary {
                let questionType = dict[ "type_id"] as! String
                return (questionType == "\(Options.TYPE_ANS.rawValue)" || questionType == "\(Options.NO_OPTION.rawValue)") ? 60 : 90
            } else {
                return 60
            }
        }
    ...
    //MARK :- add question on selection
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if tableView==listtblvw {
            ...
            surveyVMObj.arrSurveyQuestModel.append(arrQuestionsFiltered[indexPath.row])
            setDictForEdit()
            tblvw.reloadData()
        }
    }
    ...
    @objc func addMandatory(sender:UIButton){
        
        if mandForQuestion.contains(sender.tag){
            self.mandForQuestion.remove(sender.tag)
        }else{
            self.mandForQuestion.add(sender.tag)
        }
                ...
                if arrOption.count>0 {
                    ...
                    for i in 0..<arrOption.count {
                        ...
                        if dictOptions["title"] as! String == "" {
                            ...
                            //                            else {
                            //                                if i == arrOption.count-1 && ansVal == false {
                            //                                    Proxy.shared.displayStatusCodeAlert("Please enter answer for options")
                            //                                }
                            //                                boolValue = false
                            //                            }
                        }
