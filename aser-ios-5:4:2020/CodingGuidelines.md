...
```
Commit Code on Daily Basis for Project you have worked on: 
```
Steps to Create .gitignore File : 

1. Open Terminal
2. Navigate To Projects Git Repository placed on your Mac
3. Run command  touch .gitignore (It will create a .gitignore file into your project repository)
4. open .gitignore file
5. Enter the files and folder names which we don’t commit on Git

    eg: 
        Pods
        Podfile.lock
        FileName.xcworkspace
        .DS_Store
        or any other files/Folders such as framework, bundle files etc

6. Save and close File

    Files to commit

        .xcodeproj , Podfile, ProjectFilesFolder, CodingGuidelines.md, Installation.md, README.md

    Remove files From .xcodeproj file
    Steps: 
        - Select .xcodeproj files and Right Click on it
        - Select Show Package Content
        - Remove all the files except “project.pbxproj”
        
    Remove files like: xcuserdata,  xcshareddata

    Files not to Commit

    .DS_Store, Pods Folder , .xcworkspace Files , .bundle  , .framework, Podfile.lock or any of the other junk files which are generated by code while working on it.
